...
allElements.forEach(element => {
    let matches = [...element?.innerText?.matchAll(tcals)];
...
function replace_element(element, match) {
    ...
    //console.log(fileURL);
    element.innerHTML = element.innerHTML.replace(match[0], `<a download=\"tcal.ics\" href=\"${fileURL}\">`
        + match[0].substring(1, match[0].length - 1)
        + "</a>");
}
function date_time(day, month, year, hour, min) {
    const currentYear = new Date().getFullYear();
    if (year.length == 2)
        year = Math.floor(currentYear / 100) + year;
